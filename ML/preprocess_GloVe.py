...
def word_to_vector(sentences):
    corpus = Corpus()
    corpus.fit(sentences, window=20)

    embedding_model = Glove(no_components=128, learning_rate=0.01)
    embedding_model.fit(corpus.matrix, epochs=50, no_threads=8, verbose=False)
    embedding_model.add_dictionary(corpus.dictionary)
...
def get_document_vectors_in_batches(embedding_dic, sentence_label_pair):
    ...
    for sentence in sentences:
        ...
        if doc2vec is not None:
            ...
            document_embedding_list.append(tuple(doc2vec))
...
def main(num_workers, batch_size, X, Y):
    ...
    embedding_model = word_to_vector(sentences)
    ...
    for word in embedding_model.dictionary.keys():
        key_wv_dic[word] = embedding_model.word_vectors[embedding_model.dictionary[word]]
    ...
    vector_label_dic = { vector : label for vector, label in zip(input_vectors, categories)}

    with open("dataset_glove", "wb") as f:
        pickle.dump(vector_label_dic, f)
